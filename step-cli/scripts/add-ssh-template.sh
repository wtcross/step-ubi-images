#!/bin/bash
# Add an SSH certificate template to a provisioner
# Usage: add-ssh-template.sh --ca-url URL --root CERT --admin-password-file FILE --provisioner NAME --template-file FILE [OPTIONS]
set -eo pipefail
...
Add an SSH certificate template to an existing provisioner.
...
Required:
    ...
    --template-file FILE         Path to the SSH template file (JSON/Go template)
...
VOLUMES=(
    ...
    "-v" "$(realpath "${TEMPLATE_FILE}"):/tmp/ssh-template.tpl:ro"
)
...
STEP_ARGS=(
    ...
    "--ssh-template" "/tmp/ssh-template.tpl"
)
...
if [[ -n "${TEMPLATE_DATA}" ]]; then
    ...
    STEP_ARGS+=("--ssh-template-data" "/tmp/template-data.json")
fi
