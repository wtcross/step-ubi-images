#!/bin/bash
# Create a new ECDSA P-384 key on the HSM
# Usage: create-key.sh --pkcs11-socket PATH --pin-file PATH --token NAME --key-id ID --key-label LABEL [OPTIONS]
set -eo pipefail
...
exec podman run --rm "${VOLUMES[@]}" "${IMAGE}" \
    step-kms-plugin create --json --kty EC --crv P-384 \
    --kms "${TOKEN_URI}" "${KEY_URI}"
